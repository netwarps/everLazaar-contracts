...
const deployForTest = async () => {
  ...
  await createTestArticle(mainContract, kmcToken, accounts)
...
const newToken20Name = 'KmcTokenV2'
const newToken1155Name = 'ElzToken1155V2'
const newMainContractName = 'EverlazaarV2'
...
describe('================ Everlazaar Contract Test start ==============', () => {
  ...
  describe('mint-revert', async () => {
    it('Require fail - the Token1155 shall not be existed', async () => {
      ...
      const token1155 = await ethers.getContractAt('Token1155', await mainContract.kugga1155())
  ...
  describe('\npermit-create-article-revert', async () => {
    ...
    it('Require fail - the owner shall be the create article signature signer', async () => {
      ...
      const labId = await mainContract.getLabCount() - 1
      const itemHash = getRandContentHash(labId)
      ...
      await expect(mainContract.connect(caller).permitCreateArticle(itemHash, otherOne.address, v, r, s))
        .to.be.revertedWith(revertMsg.permitInvalidSignature)
    })
